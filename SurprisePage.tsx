...
const MemoryCard: React.FC<{ item: { url: string; caption: string }; index: number }> = ({ item, index }) => {
  ...
  return (
    <>
      <div 
        className={`group relative overflow-hidden rounded-2xl shadow-lg transition-all duration-700 h-[280px] sm:h-[350px] md:h-[400px] bg-white border border-rose-100/50 hover:shadow-xl hover:-translate-y-1`}
      >
        {/* Image/Video content */}
        <div className={`absolute inset-0 z-10 transition-all duration-1000 transform opacity-100 scale-100`}>
          {isVideo ? (
            <video 
              src={item.url} 
      ...
      {isZoomed && (
        ...
        >
          <button 
            ...
            className="absolute top-4 right-4 sm:top-8 sm:right-8 text-white/60 hover:text-white text-3xl sm:text-4xl font-light transition-colors z-10 hover:text-white/100"
          >&times;</button>
          <div 
            className="max-w-5xl w-full h-full flex flex-col items-center justify-center gap-4 sm:gap-8"
...
const SurprisePage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-rose-50/30 pb-16 sm:pb-32 relative selection:bg-rose-200">
      <FloatingHearts />
      <audio ref={audioRef} src={ROMANTIC_SONG_URL} loop preload="auto" crossOrigin="anonymous" />
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 mt-12 sm:mt-20 relative z-10">
        <div className="glass-card rounded-2xl sm:rounded-[3rem] overflow-hidden flex flex-col lg:flex-row items-stretch shadow-2xl border-white">
          <div className="w-full lg:w-1/2 h-[300px] sm:h-[400px] lg:h-auto min-h-[300px] sm:min-h-[400px] overflow-hidden flex items-center justify-center">
             <img 
               src={ABOUT_HER.image} 
               alt="Praveena" 
          ...
          <div className="w-full lg:w-1/2 p-8 sm:p-12 lg:p-20 text-center lg:text-left flex flex-col justify-center items-center lg:items-start">
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 mt-24 sm:mt-48 relative z-10">
        {!isVideoVisible ? (
          <div className="py-12 sm:py-20 flex flex-col items-center justify-center">
            ...
            <h2 className="text-2xl sm:text-3xl font-luxury text-rose-900 mb-6 sm:mb-8 italic px-4 text-center">I have one final surprise for you...</h2>
            <button 
          ...
          <div className="w-full flex justify-center">
            <div className="glass-card p-2 sm:p-4 rounded-2xl sm:rounded-[3rem] shadow-2xl border-rose-100/50 animate-fade-in w-full max-w-sm">
              <div className="relative rounded-xl sm:rounded-[2.5rem] overflow-hidden bg-black aspect-[9/16] shadow-inner">
